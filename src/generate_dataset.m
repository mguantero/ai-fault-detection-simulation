...
% Add a manual/simulation selection
uicontrol('Style', 'text', ...
...
% Analyze and Clear buttons - positioned below all input fields
...
% Create a compact result panel - made smaller and with centered title
result_panel = uipanel('Title', 'Detection Result', ...
                      'TitlePosition', 'centertop', ...
                      'Position', [0.3, 0.05, 0.4, 0.09], ... % Smaller panel
                      'FontSize', 9, ...
                      'BackgroundColor', [0.94, 0.94, 0.94]);

% Result text elements inside panel
result_text = uicontrol('Parent', result_panel, ...
                       'Style', 'text', ...
                       'String', '', ...
                       'Position', [10, 25, result_panel.Position(3)*700-20, 20], ... % Adjusted position and size
                       'FontSize', 12, ...
                       'HorizontalAlignment', 'center', ...
                       'BackgroundColor', [0.94, 0.94, 0.94]);

confidence_text = uicontrol('Parent', result_panel, ...
                           'Style', 'text', ...
                           'String', '', ...
                           'Position', [10, 5, result_panel.Position(3)*700-20, 15], ... % Adjusted position and size
                           'FontSize', 9, ...
                           'HorizontalAlignment', 'center', ...
                           'BackgroundColor', [0.94, 0.94, 0.94]);
...
function mode_selection_callback(source, ~)
    ...
    if selected_mode == 1
        % Manual input - do nothing
        return;
    end
    
    % Get parameters based on the selected simulation type
    switch selected_mode
        case 2  % Normal
            params = generate_params('Normal');
        case 3  % Open
            params = generate_params('Open');
        case 4  % Short
            params = generate_params('Short');
        case 5  % Mismatch
            params = generate_params('Mismatch');
    end
    
    % Update the fields with generated values
    for i = 1:6
...
function analyze_callback(~, ~)
    ...
    try
        ...
        set(result_text, 'String', ['Result Detected: ' fault_type], 'ForegroundColor', color);
        set(confidence_text, 'String', sprintf('Confidence: %.1f%%', max_score*100));
        
    catch e
        set(result_text, 'String', 'Error in analysis', 'ForegroundColor', [0.8, 0, 0]);
        set(confidence_text, 'String', e.message);
    end
end

% Preprocess input according to training data parameters
function processed_input = preprocess_input(input)
    ...
    % Standardize input using training data parameters
    processed_input = (input - preprocessing_info.mu) ./ preprocessing_info.sigma;
end

% Generate realistic parameters for each fault type
function params = generate_params(fault_type)
    % Random seed based on current time for variety
    rng('shuffle');
